...
public class TemplateRPG2D extends SimpleRolePlayingGame {
	...
	*/
	...
	private Sound3D music = BGM3D.registerBGM("data\\bgm.wav");
	private Sound3D damage = new Sound3D("data\\effect.wav");
	...
	public void progress(RWTVirtualController virtualController, long interval) {
		...
		if ((virtualController.isKeyDown(0, RWTVirtualController.BUTTON_B) && castleable == true && unitval != 0)
			...
			if (i<= 10){
			castle[i] = new Player("data\\towerdefence\\baria.gif");
			castle[i].setPosition(player.getPosition());
			castle[i].setCollisionRadius(34.0);
		...
		//for (int i = 0; i < myShipBulletList.size(); i++) {
			...
			for (int j = 0; j < enemyUnitList.size(); j++) {
				...
				if (castle[i]!=null /*&& castle[i].checkCollision(enemyUnit)*/) {
					//System.out.println("enemyのｘ座標"+enemyUnit.getPosition().getX()+"タワーのｘ座標"+castle[i].getPosition().getX());
					//System.out.println("enemyのy座標"+enemyUnit.getPosition().getY()+"タワーのy座標"+castle[i].getPosition().getY());
				}
				
				
				if (castle[i]!=null && ((castle[i].getPosition().getX()-enemyUnit.getPosition().getX()) < 3) 
						&& (castle[i].getPosition().getY()-enemyUnit.getPosition().getY()) < 3){
				//if (myBase.checkCollision(enemyUnit)) {
